 ...
 void UltraSonarCallback(const sensor_msgs::Range::ConstPtr&msg)
 {
	 ...
	 aeb_collision_distance = vx *(0.7 + 0.1) * 0.22778 * 2.5;   // 1m/sec = 0.22778km/h    2.1 = safety coefficient
...
void odomCallback(const nav_msgs::Odometry& msg)
{
	...
	old_x = x;
	old_y = y;
	ROS_INFO("X , Y : %.2lf %.2lf", msg.pose.pose.position.x, msg.pose.pose.position.y);
 ...
 int main(int argc, char **argv)
 {
	 ...
	 ros::Publisher pub_aeb_activation_flag = n.advertise<std_msgs::Bool>("/aeb_activation_flag", 10);
	 ...
	 while(ros::ok())
	 {
		 ...
		 {
			if(cmd_vel_msg.linear.x > 0) cmd_vel_msg.linear.x = 0;  // car back
